...
$envGates = [ordered]@{
    ...
    UN_DMA_SCAN_LATEST_WINS = Get-EnvGateValue 'UN_DMA_SCAN_LATEST_WINS'
    UN_DMA_COLD_TOPOLOGY_SCAN = Get-EnvGateValue 'UN_DMA_COLD_TOPOLOGY_SCAN'
    UN_DMA_STEADY_ENTITY_SCHEDULER = Get-EnvGateValue 'UN_DMA_STEADY_ENTITY_SCHEDULER'
    UN_DMA_CRITICAL_BATCH_FUSION = Get-EnvGateValue 'UN_DMA_CRITICAL_BATCH_FUSION'
...
$preflight = [ordered]@{
    ...
    runtime_gates = if ($diagnosticsProbe.ok -and $diagnosticsProbe.response.diagnostics) {
        ...
        [ordered]@{
            runtime_profile = [string]$rd.runtime_profile
            entity_pipeline_v2_enabled = if ($null -ne $rd.entity_pipeline_v2_enabled) { [bool]$rd.entity_pipeline_v2_enabled } else { $null }
            scan_latest_wins_enabled = if ($null -ne $rd.scan_latest_wins_enabled) { [bool]$rd.scan_latest_wins_enabled } else { $null }
            cold_topology_scan_enabled = if ($null -ne $rd.cold_topology_scan_enabled) { [bool]$rd.cold_topology_scan_enabled } else { $null }
            viewmatrix_poll_sleep_ms = As-Int64OrZero $rd.viewmatrix_poll_sleep_ms
            viewmatrix_effective_poll_sleep_ms = As-Int64OrZero $rd.viewmatrix_effective_poll_sleep_ms
            steady_entity_scheduler_enabled = if ($null -ne $rd.steady_entity_scheduler_enabled) { [bool]$rd.steady_entity_scheduler_enabled } else { $null }
            critical_batch_fusion_enabled = if ($null -ne $rd.critical_batch_fusion_enabled) { [bool]$rd.critical_batch_fusion_enabled } else { $null }
            critical_batch_fusion_active = if ($null -ne $rd.critical_batch_fusion_active) { [bool]$rd.critical_batch_fusion_active } else { $null }
            critical_matrix_plan_valid = if ($null -ne $rd.critical_matrix_plan_valid) { [bool]$rd.critical_matrix_plan_valid } else { $null }
            critical_matrix_plan_generation = As-Int64OrZero $rd.critical_matrix_plan_generation
            connection_epoch = As-Int64OrZero $rd.connection_epoch
            critical_matrix_plan_connection_epoch = As-Int64OrZero $rd.critical_matrix_plan_connection_epoch
...
while ((Get-Date) -lt $deadline) {
    try {
        ...
        $sample = [pscustomobject]@{
            ...
            runtime_profile = [string]$d.runtime_profile
            entity_pipeline_v2_enabled = if ($null -ne $d.entity_pipeline_v2_enabled) { [bool]$d.entity_pipeline_v2_enabled } else { $null }
            scan_latest_wins_enabled = if ($null -ne $d.scan_latest_wins_enabled) { [bool]$d.scan_latest_wins_enabled } else { $null }
            cold_topology_scan_enabled = if ($null -ne $d.cold_topology_scan_enabled) { [bool]$d.cold_topology_scan_enabled } else { $null }
            viewmatrix_poll_sleep_ms = As-Int64OrZero $d.viewmatrix_poll_sleep_ms
            viewmatrix_effective_poll_sleep_ms = As-Int64OrZero $d.viewmatrix_effective_poll_sleep_ms
            steady_entity_scheduler_enabled = if ($null -ne $d.steady_entity_scheduler_enabled) { [bool]$d.steady_entity_scheduler_enabled } else { $null }
            critical_batch_fusion_enabled = if ($null -ne $d.critical_batch_fusion_enabled) { [bool]$d.critical_batch_fusion_enabled } else { $null }
            critical_batch_fusion_active = if ($null -ne $d.critical_batch_fusion_active) { [bool]$d.critical_batch_fusion_active } else { $null }
            critical_matrix_plan_valid = if ($null -ne $d.critical_matrix_plan_valid) { [bool]$d.critical_matrix_plan_valid } else { $null }
            critical_matrix_plan_generation = As-Int64OrZero $d.critical_matrix_plan_generation
            connection_epoch = As-Int64OrZero $d.connection_epoch
            critical_matrix_plan_connection_epoch = As-Int64OrZero $d.critical_matrix_plan_connection_epoch
