...
{
    if (kmbox::KmBoxMgr.s_Client <= 0)
        return err_creat_socket;

    int Status = 0;

    kmbox::KmBoxMgr.PostData.head.indexpts++;
    kmbox::KmBoxMgr.PostData.head.cmd = cmd_mouse_move;
    kmbox::KmBoxMgr.PostData.head.rand = rand();

    this->MouseData.x = x;
    this->MouseData.y = y;

    memcpy_s(&kmbox::KmBoxMgr.PostData.cmd_mouse, sizeof(soft_mouse_t), &this->MouseData, sizeof(soft_mouse_t));
    ...
    return kmbox::KmBoxMgr.SendData(Length);
}
...
int KmBoxMouse::Move_Auto(int x, int y, int Runtime)
{
    ...
    kmbox::KmBoxMgr.PostData.head.indexpts++;
    kmbox::KmBoxMgr.PostData.head.cmd = cmd_mouse_automove;
    kmbox::KmBoxMgr.PostData.head.rand = Runtime;
...
int KmBoxMouse::Left(bool Down)
{
    ...
    kmbox::KmBoxMgr.PostData.head.indexpts++;
    kmbox::KmBoxMgr.PostData.head.cmd = cmd_mouse_left;
    kmbox::KmBoxMgr.PostData.head.rand = rand();
...
int KmBoxMouse::Right(bool Down)
{
    ...
    kmbox::KmBoxMgr.PostData.head.indexpts++;
    kmbox::KmBoxMgr.PostData.head.cmd = cmd_mouse_right;
    kmbox::KmBoxMgr.PostData.head.rand = rand();
...
int KmBoxMouse::Middle(bool Down)
{
    ...
    kmbox::KmBoxMgr.PostData.head.indexpts++;
    kmbox::KmBoxMgr.PostData.head.cmd = cmd_mouse_middle;
    kmbox::KmBoxMgr.PostData.head.rand = rand();
...
int KmBoxKeyBoard::StartMonitor(WORD Port)
{
    ...
    kmbox::KmBoxMgr.PostData.head.indexpts++;
    kmbox::KmBoxMgr.PostData.head.cmd = cmd_monitor;

    this->MonitorPort = Port;
    kmbox::KmBoxMgr.PostData.head.rand = Port | 0xaa55 << 16;

    int Length = sizeof(cmd_head_t);

    kmbox::KmBoxMgr.SendData(Length);
    ...
    return kmbox::KmBoxMgr.NetHandler();
}
