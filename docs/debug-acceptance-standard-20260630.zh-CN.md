...
`PlayerInfo drawn > 0` 只说明渲染分支被执行，不保证最终颜色 alpha 非零、敌我分类正确或屏幕上真的产生可见像素。框消失类回归必须把 team 语义、实际颜色 alpha 和 OBS/Scenario 画面放在同一时刻核对。
...
```text
所有被 DecryptComponent 消费的 type id 都必须归入 RuntimeOffsetProfile；不得只更新 rotation/link/health/hero 而让 Team 或 PlayerController 继续走全局旧常量。
Team：在已知无友军或明确混合敌我的房间验证 flags/comparison key；已知无友军却全部 ally 直接失败。
PlayerController：只允许一个本地候选，+0x1260 方向向量应可读、有限且长度接近 1；本地 hero/id 连续 30-60 秒不得反复切换。
RotationBase / RotationBase_Sub1/Sub2 / published Rot / yaw_forward 必须验证。
如果 skeleton、hitbox 或朝向显示依赖 rotation，必须有 known real hero 现场样本。
training bot 只能作为旁证，不能单独证明 real hero 的骨骼朝向正确。
```
...
旧 type id 能解出非空指针或大量 wrapper，但字段语义不成立：失败。
